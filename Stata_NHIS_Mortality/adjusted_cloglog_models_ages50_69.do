...
capture noisily svy,subpop(age_50_69): cloglog dead `base_vars', eform nolog
est store m1 

margins, at(xspd2= (1 2))
margins, dydx( xspd2)

capture noisily svy,subpop(age_50_69): cloglog dead `base_smk', eform nolog
est store m2

margins, at(xsmoke= (1 2 3) xspd2= (1 2))
margins, dydx( xspd2 xsmoke)

capture noisily svy,subpop(age_50_69): cloglog dead `base_dis', eform nolog
est store m3 

margins, at(chronic1p= (1 2) xspd2= (1 2))
margins, dydx( xspd2 chronic1p)

capture noisily svy,subpop(age_50_69): cloglog dead `base_s_d', eform nolog
est store m4
...
capture noisily svy,subpop(age_50_69): cloglog dead `base_s_d_s', eform nolog
est store m5
margins, at(xsmoke= (1 2 3) xspd2= (1 2))
margins, at(chronic1p= (1 2) xspd2= (1 2))
margins, dydx(xspd2 xsmoke chronic1p)
...
estout m1 m2  m3 m4 m5 using "E:\Stata_mortality\models_ages50_69.txt", replace ///
...
capture noisily svy,subpop(age_50_69): cloglog dead `base_bmi', eform nolog
est store m6

capture noisily svy,subpop(age_50_69): cloglog dead `base_s_dbmi', eform nolog
est store m7

capture noisily svy,subpop(age_50_69): cloglog dead `base_s_d_sb', eform nolog
est store m8

estout m6 m7 m8 using "E:\Stata_mortality\m6_7_8_ages50_69.txt", replace ///
cells("b(fmt(%5.2f) star) ci(par([  ,  ]) fmt(%6.2f))" ) eform ///
stats(N_sub, fmt(%12.0gc) labels("PY of obs")) ///
collabels("RR" "95% CI") drop(_cons) style(fixed)
