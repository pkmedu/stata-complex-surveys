...
tab2 y_age_grp SPD5  if model_obs==1 & xdead==1 & sex==1
tab2 y_age_grp SPD5  if model_obs==1 & xdead==1 & sex==2

tab2 y_age_grp SPD5  if model_obs==1 & xdead==1 
tabulate SPD5  if model_obs==1 & xdead==1 
tabulate SPD5  if model_obs==1 & xdead==1 & sex==1
tabulate SPD5  if model_obs==1 & xdead==1 & sex==2
...
display _newline _column(30) "Weighted Count - Ages 50 to 74"
svy, subpop(if model_obs==1 ): mean xd1000, over(`indvar' SPD5) 
 }

foreach indvar of varlist y_age_grp  {
display _newline _column(30) "Weighted Count - Ages 50 to 74"
svy, subpop(if model_obs==1 ): mean xd1000, over(`indvar' SPD5 sex) 
 }
