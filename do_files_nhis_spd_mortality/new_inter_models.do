...
label define cigsmoke_3c_lab ///  
 ...
 gen model_obs = !missing(y_age_grp, sex, racehisp, educ_cat, marital, ///
                          dur_cat, phy_s, alc_s,cigsmoke_3c, xspd2)

 tab model_obs dead
  tab model_obs cig_dis3 if dead==1
...
foreach depvar of varlist  lp_death lpc_death dead {
	capture noisily svy,subpop(if gen model_obs==1): cloglog `depvar' `demo' `ses ' ///
		i.xspd2 i.cigsmoke_3c , eform nolog
	}

foreach depvar of varlist  lp_death lpc_death dead {
	capture noisily svy,subpop(if gen model_obs==1): cloglog `depvar' `demo' `ses ' ///
		i.xspd2 i.cigsmoke_3c i.alc_s i.phy_s i.chronic1p, eform nolog
	}	

	foreach depvar of varlist  lp_death lpc_death dead {
	capture noisily svy,subpop(if gen model_obs==1): cloglog `depvar' `demo' `ses ' ///
		i.xspd2 i.cigsmoke_3c i.alc_s i.phy_s i.chronic1p i.xspd2##i.sex, eform nolog
	}	
	
	foreach depvar of varlist  lp_death lpc_death m_cvd_death dead {
   		capture noisily svy,subpop(if gen model_obs==1): cloglog `depvar' `demo' `ses ' ///
	...
	foreach depvar of varlist  lp_death lpc_death m_cvd_death dead {
   		capture noisily svy,subpop(if gen model_obs==1): cloglog `depvar' `demo' `ses ' ///
		i.xspd2 i.cigsmoke_3c i.xspd2##i.sex i.xspd2##i.cigsmoke_3c ///		
		i.alc_s i.phy_s i.chronic1p , eform nolog
	}	
